...
void cmd_upgrade(const std::vector<std::string>& args) {
    ...
    // If no specific packages are provided, upgrade all installed packages
    if (args.empty()) {
        print_message("Upgrading all installed packages is not implemented yet", YELLOW);
    ...
    // For each package in the list, check if an upgrade is available and install it
    for (const std::string& pkg : packages) {
        ...
        if (find_package(pkg, package, pkgname, pkgver, pkgarch)) {
            if (is_package_installed(pkgname)) {
                ...
                if (CPK_VERBOSE) {
                    print_message("Package " + pkgname + " is not installed, skipping", YELLOW);
                }
            }
        }
        else {
            if (CPK_VERBOSE) {
                print_message("Package " + pkg + " not found in index, skipping", YELLOW);
            }
        }
    }
